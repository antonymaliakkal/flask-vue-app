<template>
    ...
    <div>
    <div class="main">
        <div class="sub1" v-for="key in cat" :key="key">
            {{ key.value.name }}
            <br><br>
            {{ key.value.desc }}
    ...
    export default {
        ...
        methods : {
            ...
            savechanges() { 
                ...
                axios.patch('http://localhost:5000/edit_category' , this.editedCategory)
                    .then(response => {
                        this.isModalVisible = false;
            ...
            deleteCategory(category) {
                console.log(category)
                axios.post('http://localhost:5000/delete_category' , {cat : category})
                    .then(response => {
                        console.log(response.data['message'])
                        window.location.reload();
